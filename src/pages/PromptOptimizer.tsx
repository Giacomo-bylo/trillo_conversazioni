...
import { Sparkles, Check, Wand2, GitCompare } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col overflow-hidden">
           <div className="p-4 border-b border-gray-200 bg-gray-50 font-medium text-gray-700">
             1. Seleziona Chiamate di Riferimento
           </div>
           <div className="overflow-y-auto flex-1 p-2">
             {calls.map(call => (
               <div 
                 ...
                 className={cn(
                   "p-3 rounded-md mb-2 cursor-pointer border transition-all",
               ...
               >
                 <div className="flex justify-between items-start">
                   <div>
                     <div className="font-medium text-sm text-gray-900">{call.lead_nome}</div>
                     <div className="text-xs text-gray-500">{call.score?.quality_score}/10 Quality Score</div>
                   </div>
                   {selectedCalls.includes(call.id) && <Check size={16} className="text-primary" />}
                 </div>
                 {call.score?.identified_issues && call.score.identified_issues.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                       {call.score.identified_issues.slice(0, 2).map((iss, i) => (
                          <span key={i} className="text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded">{iss}</span>
                       ))}
                    </div>
                 )}
               </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col">
           ...
           <div className="p-4 flex-1 flex flex-col gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                   {QUICK_ACTIONS.map((action) => (
                      <button 
                        ...
                        className="text-xs px-3 py-1.5 bg-white border border-gray-300 rounded-full hover:border-primary hover:text-primary transition-colors"
                      >
                        {action}
                      </button>
                   ))}
              ...
              <button 
                disabled={isAnalyzing || !instruction}
